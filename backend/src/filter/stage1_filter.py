...
class Stage1Filter:
    ...
    def __init__(
        ...
        logger.info(f"Stage1Filter initialized: threshold={threshold}, temperature={temperature}")
    ...
    async def filter_batch(
        ...
        if uncached_papers:
            ...
            results = await self.llm_client.complete_batch(
                batch_messages, Stage1Result, temperature=self.temperature
            )
            ...
            for paper, messages, result in zip(
                ...
                if result is None:
                    ...
                    logger.warning(
                        f"Stage 1: Paper {paper['id']} failed LLM call, marking as not passed"
                    )
                    result_dict = {
                        "pass_filter": False,
                        "score": 0.0,
        ...
        logger.info(
            f"Stage 1 complete: {passed}/{len(papers)} papers passed ({passed / len(papers) * 100:.1f}%)"
        )
